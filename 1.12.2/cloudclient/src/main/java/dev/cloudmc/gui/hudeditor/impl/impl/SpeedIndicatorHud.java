...
public class SpeedIndicatorHud extends HudMod {
        ...
        if (Cloud.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Cloud.INSTANCE.mc.fontRenderer.drawString(
                        getMeterPerSecond() + " m/s",
                        getX() + getW() / 2 - (Cloud.INSTANCE.mc.fontRenderer.getStringWidth(getMeterPerSecond() + " m/s")) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private double getMeterPerSecond() {
        double x = Cloud.INSTANCE.mc.player.posX - Cloud.INSTANCE.mc.player.prevPosX;
        double z = Cloud.INSTANCE.mc.player.posZ - Cloud.INSTANCE.mc.player.prevPosZ;
        double speed = net.minecraft.util.math.MathHelper.sqrt(x * x + z * z) * 20;

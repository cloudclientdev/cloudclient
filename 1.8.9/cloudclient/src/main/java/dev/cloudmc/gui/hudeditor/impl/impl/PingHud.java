...
public class PingHud extends HudMod {
        ...
        if (Cloud.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Cloud.INSTANCE.fontHelper.size20.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2f - Cloud.INSTANCE.fontHelper.size20.getStringWidth("Ping: " + getPing()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Cloud.INSTANCE.mc.fontRendererObj.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2 - Cloud.INSTANCE.mc.fontRendererObj.getStringWidth("Ping: " + getPing()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private long getPing() {
        return Cloud.INSTANCE.mc.getCurrentServerData() == null ? 0 : Cloud.INSTANCE.mc.getCurrentServerData().pingToServer;
    }
